...
if (!deployUrl) {
  ...
  deployUrl = process.env.CODEBUFF_DOWNLOAD_BASE;
}
...
if (!deployUrl) {
  ...
  console.error("2. Set CODEBUFF_DOWNLOAD_BASE in .env.local");
  process.exit(1);
}

// Extract port from CODEBUFF_DOWNLOAD_BASE
...
const modifiedPostinstall = originalPostinstall.replace(
  ...
  process.env.CODEBUFF_DOWNLOAD_BASE,
);
...
sh("bun build src/index.ts --compile --outfile ../npm-installer/bin/codebuff", {
  cwd: join(here, "npm-app"),
});
...
const bundleDir = join(here, "bundles");
...
if (plat === "win32") {
  ...
  sh(`tar -C npm-installer/bin -czf ${tarPath} codebuff`);
  ...
  if (stats.size === 0) {
    throw new Error(`Created tarball is empty: ${tarPath}`);
  }
...
const testUrl = `${process.env.CODEBUFF_DOWNLOAD_BASE}/codebuff-${plat}-${arch}.${plat === "win32" ? "zip" : "tar.gz"}`;
...
sh(`npm --foreground-scripts install -g ${tarball}`, {
  env: {
    ...process.env,
    CODEBUFF_DOWNLOAD_BASE: process.env.CODEBUFF_DOWNLOAD_BASE,
