...
async function main() {
  ...
  try {
    ...
    // Step 2: Add new content to slide 2 - FIXED: create shapes and text in same batch
    console.log('Adding new content to slide 2...');
    const titleId = 'title_' + Date.now();
    const bodyId = 'body_' + (Date.now() + 1);
    ...
    const title3Id = 'title3_' + Date.now();
    const body3Id = 'body3_' + (Date.now() + 1);
