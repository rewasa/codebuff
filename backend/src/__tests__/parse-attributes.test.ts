...
describe('parseAttributes', () => {
  ...
  it('should handle extra whitespace', () => {
    const result = parseAttributes('  name  =  "value"    id="123"  ', [
      'name',
      'id',
    ])
    expect(result).toEqual({
  ...
  it('should handle attributes with special characters', () => {
    const result = parseAttributes('path="x=1&y=2" query="a>b"', [
      'path',
      'query',
    ])
    expect(result).toEqual({
  ...
  it('should handle attributes with spaces in values', () => {
    const result = parseAttributes('name="John Doe" title="Senior Developer"', [
      'name',
      'title',
    ])
    expect(result).toEqual({
  ...
  it('should handle backslashes in attribute values', () => {
    const result = parseAttributes('path="C:\\folder\\file.txt" id="123"', [
      'path',
      'id',
    ])
    expect(result).toEqual({
  ...
  it('should handle unicode characters in values', () => {
    const result = parseAttributes('name="🙂 José" lang="español"', [
      'name',
      'lang',
    ])
    expect(result).toEqual({
