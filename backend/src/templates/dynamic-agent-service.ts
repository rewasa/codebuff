...
export class DynamicAgentService {
  ...
  async loadAgents(
    ...
    // Filter out TypeScript declaration files (.d.ts) that are generated by agent builder
    // Note: Frontend strips .ts extension from agent template keys, so we need to handle both cases
    // Filter out .d.ts files but allow both .ts files and keys without extensions
    const agentTemplates = Object.fromEntries(
      Object.entries(allAgentTemplates).filter(([key]) => {
        // Allow keys that don't end with .d.ts (either .ts files or keys without extensions)
        return !key.endsWith('.d.ts')
      })
    )
