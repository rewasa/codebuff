...
export class Cursor {
  ...
  left(skipNewline: boolean = false): Cursor {
    if (this.isAtStart()) {
      return this;
    }
    let newOffset = this.offset - 1;
    // Skip over newline if we land on one, unless we're moving through chars
    if (!skipNewline && this.text[newOffset] === "\n") {
      newOffset--;
    }
    return new Cursor(this.measuredText, Math.max(0, newOffset));
  }

  right(skipNewline: boolean = false): Cursor {
    if (this.isAtEnd()) {
      return this;
    }
    let newOffset = this.offset + 1;
    ...
    return new Cursor(this.measuredText, Math.min(this.text.length, newOffset));
  }
  ...
  nextWord(): Cursor {
    ...
    while (nextCursor.isOverWordChar() && !nextCursor.isAtEnd()) {
      nextCursor = nextCursor.right(true);
    }
  ...
  prevWord(): Cursor {
    // Special case: if we're right next to a newline, just move left
    ...
    if (!cursor.left(true).isOverWordChar()) {
      cursor = cursor.left(true);
    }
    ...
    if (cursor.isOverWordChar()) {
      while (cursor.left(true).isOverWordChar() && !cursor.isAtStart()) {
        cursor = cursor.left(true);
      }
    }
